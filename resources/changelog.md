# 更新日志

## v3.2.4 - 2026-06-02

**稳定维护版本**
本软件将不会继续快速更新，进入稳定维护阶段。感谢大家一直以来的支持！

**Bug 修复（12 项）：**
- 修复新手教程遮罩不随窗口缩放（Bug #1）
- 修复游戏开始按钮不重置状态（Bug #2）
- 修复发送消息信号时序错误：先断开信号再发送消息（Bug #3）
- 修复问答「再来一轮」信号累积（Bug #4）
- 修复 DeepSeek 模型写入错误的下拉框（Bug #5）
- 修复启动时 DeepSeek 模型检测结果未持久化（Bug #6）
- 修复 Windows 深色模式检测使用已弃用注册表路径，改用 SystemUsesLightTheme（Bug #7）
- 修复游戏随机数未初始化种子（Bug #8）
- 修复聊天历史存储 HTML 导致主题切换后样式不匹配（Bug #9）
- 修复首次运行向导主题默认选项应跟随系统而非当前系统模式（Bug #11）
- 修复窗口关闭前防抖设置未保存（Bug #12）
- 修复 firstrundialog.cpp 中的 systemDarkMode() 同样使用已弃用路径（Bug #7 同步）

---

## v3.2.3 - 2026-05-17

**Bug 修复（28+ 项）：**
- 修复 AI 超时定时器误杀新请求：引入 `m_requestSerial`/`m_currentSerial` 序列号机制
- 修复流式响应末尾不触发 `responseReceived`：添加兜底 `emit responseReceived("")`
- 修复 `onChatReplyFinished` 中 `processBuffer` 调用顺序导致的尾部丢失
- 修复聊天记录无限累积：改用 `QStringList m_chatHistory` 限制 100 条上限
- 修复 `processInline` 中行内代码内粗体/斜体被错误解析：改用不透明占位符方案
- 修复 Markdown 渲染中转义顺序导致的 `<b>` 标签被编码
- 修复 `generateQuiz` 中 `onErrorOccurred` 信号泄漏：同时断开聊天模式的 error 连接
- 修复 `sendChatMessage` 中仅断开了答题信号但未断开同级同名信号
- 修复 `saveSettings` 中保存"自定义"字面量作为模型名导致 Ollama 请求失败
- 修复 `onModelsFetched` 中竞态问题：切换服务商后模型列表不恢复、provider 读取不一致
- 修复主窗口构造中 `disconnect(btn, nullptr, this, nullptr)` 过于宽泛的问题
- 修复知识答题选项前缀重复：数据中已含"A."前缀，不再额外添加
- 修复 AI 回答未存入聊天历史：`onResponseReceived` 中追加到 `m_chatHistory`
- 修复文本框频繁写盘导致 UI 卡顿：引入 500ms 防抖定时器
- 修复游戏 `resetGame` 未重置 `m_paused` 状态：点击重新开始后仍显示暂停
- 修复教程遮罩 `mapToGlobal` 坐标偏移导致高亮区域错位的问题
- 修复教程点击任意位置前进：改为仅点击下一按钮才前进，点击目标区域穿透下层控件
- 修复模型选择下拉框不可编辑：设置为可编辑并添加 placeholder
- 修复 AI 回复重复出现：引入 `m_streamFinished` 防止 `responseReceived` 双重发射
- 修复 AI 回复时切换到答题导致闪退：`sendMessage` 中先置 `m_currentReply = nullptr` 再 `abort()`，防止 `onChatReplyFinished` 回写后空指针崩溃
- 修复有序列表渲染只有"1"：改用手动编号避免 QTextEdit `<ol>` 渲染缺陷
- 修复 Markdown 渲染缺少引用块和分隔线支持
- 修复行内代码中 `**` 等标记被错误渲染：改为先将代码内容 HTML 转义再存入不透明占位符

**代码改进：**
- 移除 `main.cpp`、`settingsmanager.cpp`、`firstrundialog.cpp` 中的重复代码路径
- 统一设置路径入口为 `SettingsManager::defaultSettingsPath()`
- 设置文件保存到程序目录（settings.ini），不再依赖 AppData
- 多处 `saveSettings` 调用改为防抖方式，提升性能

**界面改进：**
- 滚动条改造为现代窄条样式（8px 圆角滑块），适配深色/浅色主题

---

## v3.2.2 - 2026-05-17

**重大修复：**
- 修复 AI 服务管理器中的缓冲区竞态条件：`abort()` 触发前先清空缓冲区，防止旧响应污染新请求
- 修复 AI 答题模式下聊天信号处理函数误触发导致界面错乱的问题
- 修复首次运行向导内存泄漏：`createFeaturesPage()` 中重复创建布局
- 修复游戏物理计时器不使用真实耗时的问题：改用 `QElapsedTimer`，消除系统负载高时的慢动作
- 修复游戏无人机 `hp` 字段无效的问题：实际应用血量并随波次递增
- 修复聊天历史因子串匹配被意外清空的问题
- 修复答题完成按钮过度使用 `disconnect(btn, nullptr, this, nullptr)` 的问题，改为精确断开
- 修复许可协议页验证失败后复选框文字始终为红色不恢复的问题

**设置系统重构：**
- 设置文件存储位置变更：仅当前用户安装 → `%LOCALAPPDATA%/DefenseEdu/settings.ini`
- 全用户安装 → `%PROGRAMDATA%/DefenseEdu/settings.ini`
- 通过 `.install-scope` 标记文件自动判断作用域

**安装程序改进：**
- 安装包运行时提供「安装给所有用户」和「仅安装给当前用户」的选择
- 非管理员用户自动跳过安装范围选择（强制仅当前用户）
- 卸载时同时清理 AppData 和 ProgramData 下的设置文件

**移除重复/死代码：**
- 移除从未被使用的 `OllamaManager` 类（`ollamamanager.cpp/h`），`AiServiceManager` 已覆盖全部功能
- 移除 `MainWindow` 中声明的无用成员 `m_cardAnims`

**网络层改进：**
- AI 服务请求添加 60 秒超时机制，防止无限等待
- 流式响应添加 1MB 缓冲区上限，防止异常响应导致内存溢出

**精简优化：**
- 游戏栏样式跟随主题，不再固定深色
- 时间轴拖动/缩放添加边界限制，防止无限滑动
- 教程遮罩点击事件简化：点击任意位置前进
- 单实例服务名添加用户名后缀，避免多用户冲突
- 多个文件中的重复 `systemDarkMode()` 函数已合并

---

## v3.2.1 - 2026-05-16

**新功能：**
- 单实例启动时自动唤醒已有窗口
- 激光防御模拟增加「开始/暂停/继续」按钮
- AI 问答输出支持 Markdown 渲染

**Bug 修复：**
- 修复首次运行向导「上一步」按钮文字被裁剪
- 修复装备分类标签宽度不足
- 修复知识问答卡片标题样式异常

**优化：**
- 移除 14px 及以下粗体，提升可读性
- 统一增大卡片标题字号

---

## v3.2.0 - 2026-05-15

**崩溃修复：**
- 修复侧边栏打开后程序崩溃：删除 QGraphicsOpacityEffect 透明度动画，避免 use-after-free 问题
- 修复页面切换时潜在的崩溃风险：移除页面淡入动画中的 QGraphicsOpacityEffect race condition
- 修复 exe 属性描述乱码：资源文件改用 ASCII 编码描述文字

**功能改进：**
- 单实例运行：同时启动两个实例时，第二个实例自动退出，不创建新窗口
- 侧边栏新增自动收回选项：设置页可勾选"导航后自动收回侧边栏"，默认开启
- 新增亚克力效果开关：设置页可启用 Win11 亚克力风格（半透明背景）
- 首次启动新增视觉效果设置页：可配置亚克力效果和侧边栏自动收回

**视觉优化：**
- 侧边栏动画升级为 OutBack/InBack 缓动曲线，动画更生动
- 侧边栏动画时长调整为 350ms（打开）/ 300ms（关闭）
- 主题按钮增大尺寸并添加 padding，防止文字被遮盖
- 首次启动上一步按钮改为自适应宽度并添加 padding，防止文字被遮盖
- 主界面标题从"cn国防装备"改为"中国国防装备"
- 删除无用的字体大小调节功能

**性能优化：**
- 优化侧边栏动画：复用动画对象，避免每次创建新对象导致内存泄漏
- 优化答题模块信号连接：使用 Qt::UniqueConnection 避免重复连接/断开信号
- 移除答题完成后不必要的信号重连逻辑
- 移除 Windows 原生亚克力 API 调用（导致标题栏消失），改用 Qt 半透明背景

**工具脚本：**
- 新增 build.bat 一键编译部署脚本

---

## v3.1.1 - 2026-05-05

**重大改进：**
- 全新交互式新手教程：取代原有的说明书式教程，改为沉浸式引导体验
- 教程采用遮罩高亮设计：目标按钮高亮显示，其他区域暗化处理
- 点击引导按钮即可进入下一步，直观易懂
- 教程自动导航：根据步骤自动切换页面、打开侧边栏，无需手动操作
- 教程涵盖所有核心功能：菜单按钮、首页装备、时间轴、AI问答、AI答题、知识问答、知识答题、激光防御、软件设置

---

## v3.1.0 - 2026-04-24

**优化内容：**
- 优化侧边栏导航按钮样式和布局
- 改进版本号显示至 v3.1.0

---

## v3.0.3 - 2026-04-23

**新增功能：**
- 新增新手教程功能，首次启动时自动引导
- 教程涵盖所有核心功能：装备展示、时间轴、AI问答、知识问答、答题、游戏、设置
- 教程支持上一步/下一步导航，右下角可跳过
- 教程完成后自动进入主界面

**优化内容：**
- 优化初次设置流程，设置完成后自动启动新手教程
- 升级版本号至 3.0.3

---

## v3.0.2 - 2026-04-23

**新增功能：**
- 新增DeepSeek AI服务商支持，可选择Ollama或DeepSeek作为AI服务提供者
- 新增AI设置页面，支持选项卡切换AI服务商
- DeepSeek模式支持自定义API密钥和API地址
- Ollama模式无需填写API密钥

**优化内容：**
- 将Ollama设置更名为AI设置
- 更新日志改为读取本地changelog.md文件，方便维护
- 增大设置页面的更新日志按钮，更易点击
- 优化AI问答和AI出题功能，支持不同服务商

**修复内容：**
- 修复首次设置完成后程序崩溃问题（段错误）
- 修复更新日志显示异常（$1符号问题）

---

## v3.0.1 - 2026-04-22

**修复内容：**
- 修复 AI 聊天汉字重复问题（中国变成中中国国）
- 修复知识问答页面字体渲染重影问题
- 修复答题模式信号重复连接导致的响应异常
- 优化主题切换性能，减少不必要的组件刷新

**优化内容：**
- 优化知识问答卡片样式，增大字体和内边距
- 优化设置页面布局，添加更新日志入口
- 减小编译后文件体积

**已知问题：**
- 极少数情况下 AI 流式响应可能出现短暂延迟

---

## v3.0.0 - 2026-04-21

**新增功能：**
- 新增知识问答页面（预制 12 道国防知识问答）
- 新增知识答题页面（预制 20 道选择题）
- 新增初次设置向导（首次启动时显示）
- 新增主题选择（深色/浅色/跟随系统）
- 新增字体大小调节功能
- 新增用户许可协议确认
- 新增功能介绍和使用教程

**优化内容：**
- 升级版本号至 3.0.0

---

## v2.0.0

**新增功能：**
- 新增激光防御模拟游戏
- 新增时间轴页面（原时间线）

**优化内容：**
- 将时间线更名为时间轴
- 重构渲染路线，不再使用 QtWebEngine
- 优化整体性能和启动速度

---

## v1.0.0

**初始版本：**
- 国防装备展示（9 种主力装备）
- 发展历程时间线
- AI 智能问答
- AI 智能答题
- 软件设置
